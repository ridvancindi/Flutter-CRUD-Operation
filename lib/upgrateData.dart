...
class upgrateData extends StatefulWidget {
   Product product;
  upgrateData(this.product);

  @override
  _upgrateDataState createState() => _upgrateDataState(product);
}
...
{"id":"35","name":"İZMİR"}
  ...
  Product? product;
  List<Map> ilceler = [];
  _upgrateDataState(this.product);
  int? num;
  bool vis = true;
  List<Map> country = [];
  ...
  var _schaffoldKey = GlobalKey<FormState>();
  bool visables1 = true;
  ...
  void initState(){
    ...
    if (sehir == 0) {
      visables1 = false;
      visables2 = true;
      _companytitle.text = product!.companyTitle;
      _taxdai.text = product!.taxAdministration;
      _taxid.text = product!.taxNo;
      _companyphone.text = product!.companyPhone;
      _phone.text = product!.phone;
      _mail.text = product!.email;
      _adress.text = product!.address;

    } else {
      _name.text = product!.name;
      _surname.text = product!.lastName;
      _phone.text = product!.phone;
       _tc.text = product!.taxNo;
       _mail.text = product!.email;
       _adress.text = product!.address;
                              ...
                              child: Padding(
                                ...
                                child: TextFormField(
                                  ...
                                  validator: (kontroledilecekname) {
                                    ...
                                    } else if (kontroledilecekname.length < 4) {
                                      return "4 Karakterden Küçük Olamaz...";
                                    } else
                                      return null;
                                  },
  ...
  void _upgrateData(Product product) async {
    var sonuc = await _databaseHelper!.dataUpgrate(product);
    if (sonuc == 1){
      Navigator.of(context).pop(true);
    }
  }
  ...
  ilceler.clear();
  var gelenilceJson = await DefaultAssetBundle.of(context).loadString("assets/json/ilce.json");
    ...
    for (int i = 0; i < jsonilceList["data"].length; i++) {
      if (jsonilceList["data"][i]["il_id"]== il) {
          ilceler.add({"id":jsonilceList["data"][i]["id"], "name":jsonilceList["data"][i]["name"]});
      }
    }
